...
Nach der Auswertung öffnet sich der Dialog **„Kreisliste-Scan — Vorschau"** mit bis zu zwei Gruppen:

- **Aktualisierte Kreise** — bereits vorhandene Kreise, bei denen sich laut Scan etwas geändert hat
- **Neue Kreise** — im Scan erkannte Kreise, die in der Show noch nicht existieren

Jede erkannte Änderung lässt sich **einzeln abwählen**, bevor sie übernommen wird. Der Button **„Alle umschalten"** kehrt die Auswahl aller Zeilen auf einmal um.

Wurden im Foto keine ausgefüllten Zeilen erkannt, erscheint der Hinweis „Keine ausgefüllten Zeilen erkannt."

## Übernehmen

Klick auf **„Übernehmen"** wendet nur die ausgewählten Änderungen an. Eine Bestätigung zeigt, wie viele Kreise aktualisiert und wie viele neu angelegt wurden.

::: warning Foto wird an die Anthropic-API übertragen
Der Scan nutzt ein KI-Modell von Anthropic (Claude Vision). Das hochgeladene Foto wird dafür an die Anthropic-API übertragen — anders als bei den übrigen Funktionen der Web-App, die ausschließlich mit dem eigenen LuxStage-Server kommunizieren. Details dazu in der [Datenschutzerklärung](../datenschutz#kreisliste-scannen-ki-foto-scan).
:::
